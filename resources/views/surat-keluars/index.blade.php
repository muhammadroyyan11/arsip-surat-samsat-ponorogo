...
<div class="modal fade" id="modalForm" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content border-0 shadow">
            <form id="formData" class="ajax-form" action="{{ route('surat-keluars.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div id="methodContainer"></div>
                <div class="modal-header bg-light">
                    <h5 class="modal-title fw-bold" id="modalTitle">Tambah Surat Keluar</h5>
...
<script>
    $(document).ready(function() {
        $('#datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('surat-keluars.index') }}",
            columns: [
        ...
        $(document).on('click', '.btn-edit', function() {
            ...
            $('#modalTitle').text('Edit Surat Keluar');
    ...
    function resetForm() {
        $('#modalTitle').text('Tambah Surat Keluar');
        $('#formData').attr('action', "{{ route('surat-keluars.store') }}");
